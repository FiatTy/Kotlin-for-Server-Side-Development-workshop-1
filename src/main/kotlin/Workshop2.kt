...
//🚨    println("ผลรวมราคาสินค้า Electronics ที่ราคา > 500 บาท: $totalElecPriceOver500Sequence บาท")
    println("ผลรวมราคาสินค้า Electronics ที่ราคา > 500 บาท: $totalElecPriceOver500Sequence บาท")
    println("--------------------------------------------------")
    println("กลุ่มสินค้าราคาตาม")
    val PriceUnder1000 = products
        .filter { it.price < 1000.0 }
        .map{it.name +" ราคา " + it.price + "\n"}.joinToString { it }
    val PriceBetawin1000to9999 = products
        .filter { it.price >= 1000.0 && it.price <= 10000.0 }
        .map{ it.name +" ราคา " + it.price + "\n"}.joinToString { it }
    val PriceOver10000 = products
        .filter { it.price > 10000.0 }
        .map{ it.name +" ราคา " + it.price + "\n"}.joinToString { it }

    println("ราคาไม่เกิน 1,000 บาท\n $PriceUnder1000")
    println("ราคาอยู่ระหว่าง 1,000 - 9,999 \n $PriceBetawin1000to9999")
    println("ราคาเกิน 10,000 บาท \n $PriceOver10000")
